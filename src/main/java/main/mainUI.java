...
public class mainUI extends JDialog {
    ...
    private FileNameExtensionFilter csvfilter = new FileNameExtensionFilter("CSV and TXT files", "csv", "txt");
     ...
     * If no script file has been specified, it will prompt the user to do so.
     */
    private void onEditScript() {
        if (scriptFile == null) {
            int option = JOptionPane.showConfirmDialog(null, "No scipt file selected.\nWould you like to select one now?", "No script file.", JOptionPane.YES_NO_OPTION);
    ...
    /**
     * Opens the default system program to view the directory.
     * If no output directory has been specified, it will prompt the user to do so.
     */
    private void onViewOutputDir() {
        if (outputDir == null) {
            int option = JOptionPane.showConfirmDialog(null, "No output directory selected.\nWould you like to select one now?", "No output directory.", JOptionPane.YES_NO_OPTION);
            if (option == JOptionPane.YES_OPTION) {
                onSelectOutputDir();
            } else {
                return;
    ...
    /**
     * Opens a open file dialog for the user to select a data file and a settings file.
     * When the user is done, the file wil be displayed in the files table.
     */
    private void onAddNewFile() {
        ...
        DataFile file = new DataFile(dataFile, settingsFile);
    ...
    private void onRemoveFile() {
        ...
        //Sort the index list, rows with a higher index should be removed before rows with a lower index to prevent indices from changing.
    ...
    private void onRunScript() {
        ...
        if (scriptFile == null || scriptFile.exists() == false || outputDir == null || outputDir.exists() == false || files.isEmpty()) {
            JOptionPane.showMessageDialog(null, "Please make sure you have selected a script file, output directory and at least 1 data file.", "Wrong input.", JOptionPane.INFORMATION_MESSAGE);
            return;
        }
