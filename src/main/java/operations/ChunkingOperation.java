...
public class ChunkingOperation extends Operation{
	...
	/**
	 * The settings of the inputData.
	 * We will add an extra column for the Exporter.class
	 */
	Settings settings;
	/**
	...
	/**
	 * Constructor that calls the Operation class to set the inputData and create an the colums for the new table.
	 * @param input
	 */
	...
	/**
	 * We create the chunk with new index and label if chunkingOperation returns false. 
	 * We add the a record to the chunk if chunkingOperation returns true.
	 */
	@Override
	 ...
	 * @param cce, the enum on we which to chunk.
	 * @param recordValue, the current record we need to check.
	 * @param check, the record from the current chuck by which we will compare the recordValue
	 * @return
	 */
	...
	public boolean chunkingOperation(ChunkComparatorEnum cce, Value recordValue, Value check) {
		...
		/**
		 * Switch case for determining if a new chunk is needed.
		 * Because of the cases we can cast without throwing cast exceptions. 
		 */
		switch(cce){
		case DAY : {
