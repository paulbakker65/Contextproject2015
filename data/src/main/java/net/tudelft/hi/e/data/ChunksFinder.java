...
public class ChunksFinder {
   ...
   * @param chunkDepth the depth to return the lists from.
   ...
   * 
   * @param table the table to search.
   * @param chunkDepth the depth to return the chunks from.
  ...
  public static int maxDepth(Table table) {
    return maxDepthChunk(table.getChunks());
  }
   ...
   * @param chunkDepth the depth to search at
  ...
  private static int maxDepthChunk(List<Chunk> chunks) {
    if (chunks.isEmpty()) {
      return 0;
    }
    ...
    for (Chunk chunk : chunks) {
      int depth = maxDepthChunk(chunk.getChunks());
      if (depth > max) {
        max = depth;
      }
    }
   ...
   *
   * @param table the table to search.
   * @param depth the depth to return the chunklists from.
  ...
  private static List<List<Chunk>> getChunksListAtDepth(List<Chunk> chunks, int depth) {
    if (depth == 0) {
