...
public class NotPattern extends Pattern {
   ...
   * 
   * @param invertPattern the Pattern that should not be found.
   */
  ...
  public boolean findPattern(Table table, int fromIndex, Table records) {
    ...
    Table invertRecords = new Table(records);
    if (invertPattern.findPattern(table, fromIndex, invertRecords)) {
      records.clear();
      return false;
    }
  ...
  public boolean isNextLastPattern() {
    return nextPattern.isNextLastPattern();
  }
