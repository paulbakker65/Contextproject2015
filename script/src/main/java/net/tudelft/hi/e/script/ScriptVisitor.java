...
/**
 * Script visitor class that visits each node of the parse thee instead of
 * listening to a treewalkers findings.
 */
class ScriptVisitor extends AbstractParseTreeVisitor implements
    AnalysisLangVisitor {

  private static final Logger LOG = Logger.getLogger(ScriptVisitor.class.
      getName());
  ...
  /**
   * Creates a list of operations walking a parsetree generated by the
   * analysisparser.
   *
  ...
  /**
   * The getTableForTableName method retrieves the Table with the specified
   * table name from the list of Tables in the tables field.
   *
   * @param tableName The table name that is searched for.
   * @return The Table object retrieved from tables.
   * @throws TableNotFoundException If the the table cannot be found in the
   * tables field a TableNotFoundException is thrown.
   */
  private Table getTableForTableName(final String tableName) throws
  ...
  @Override public final BetweenOperation visitBetween_operation(
      Between_operationContext ctx) {
    return visitBetween_param(ctx.param);
  }

  @Override public final BetweenOperation visitBetween_param(
      Between_paramContext ctx) {
    try {
      return new BetweenOperation(getTableForTableName(
          visitField(ctx.eventcol)[0]),
          visitField(ctx.datecol1)[1], visitField(ctx.datecol2)[1], visitValue(
              ctx.value1),
  ...
  @Override public final Object visitCalc_operator(
      AnalysisLangParser.Calc_operatorContext ctx) {
    return ctx.op;
  }

  @Override public final ChunkingOperation visitChunk_operation(
      Chunk_operationContext ctx) {
    return visitChunk_param(ctx.param);
  }

  @Override
  public final ChunkingOperation visitChunk_param(
  ...
  @Override public final ChunkType visitChunk_type(
      AnalysisLangParser.Chunk_typeContext ctx) {
    switch (ctx.i) {
      case 0:
  ...
  @Override public final Object visitCode_operation(
      AnalysisLangParser.Code_operationContext ctx) {
    return visitCode_param(ctx.param);
  }

  @Override public final CodingOperation visitCode_param(
      AnalysisLangParser.Code_paramContext ctx) {
    try {
      return new CodingOperation(
          getTableForTableName(visitTable(ctx.tableparam)),
          visitPattern(ctx.patternparam), visitText(ctx.codenameparam).
          toString());
    } catch (TableNotFoundException ex) {
      LOG.log(Level.SEVERE, ex.getMessage(), ex);
      return null;
  ...
  @Override
  public final Object visitCompare_operator(
  ...
  public final ComputeOperation visitCompute_param(
    ...
    try {
      return new ComputeOperation(getTableForTableName(
          visitTable(ctx.tableparam)),
          visitCompute_operator(ctx.computeopparam),
          visitField(ctx.fieldparam)[1]);
    } catch (TableNotFoundException ex) {
      LOG.log(Level.SEVERE, ex.getMessage(), ex);
      return null;
    }
  ...
  @Override public final List<Condition> visitCondition(
      AnalysisLangParser.ConditionContext ctx) {
  ...
  @Override public final ConstraintOperation visitConstraint_param(
    ...
    try {
      return new ConstraintOperation(getTableForTableName(visitField(
          ctx.fieldparam)[0]),
          visitField(ctx.fieldparam)[1], ctx.opparam.op, visitValue(
              ctx.valueparam));
    } catch (TableNotFoundException ex) {
      LOG.log(Level.SEVERE, ex.getMessage(), ex);
      return null;
  ...
  @Override public final ForEachChunkOperation visitForeach_chunk_param(
    ...
    try {
      return new ForEachChunkOperation(getTableForTableName(visitTable(
          ctx.tableparam)),
          visitNumberGetInt(ctx.levelparam), visitOperationNoAdd(
              ctx.operationparam));
    } catch (TableNotFoundException ex) {
      LOG.log(Level.SEVERE, ex.getMessage(), ex);
      return null;
  ...
  @Override public final LsaOperation visitLsa_param(
      AnalysisLangParser.Lsa_paramContext ctx) {
    try {
      String[] fieldReference = visitField(ctx.fieldparam);
      return new LsaOperation(getTableForTableName(fieldReference[0]),
          fieldReference[1],
          visitNumberGetInt(ctx.from), visitNumberGetInt(ctx.to), visitValue(
              ctx.key),
  ...
  @Override public final Operation visitOperation(
  ...
  @Override
  public final Object visitRecord_condition(
